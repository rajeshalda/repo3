...
try {
    ...
    Write-Host "Please manually install URL Rewrite Module from https://www.iis.net/downloads/microsoft/url-rewrite" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "Please manually install ARR from https://www.iis.net/downloads/microsoft/application-request-routing" -ForegroundColor Yellow
}
...
$indexContent | Out-File -FilePath "$websitePath\index.html" -Encoding UTF8
...
try {
    ...
    Write-Host "Manual SSL binding required in IIS Manager" -ForegroundColor Yellow
}

# Step 7: Configure Application Request Routing at server level
Write-Host "Configuring Application Request Routing..." -ForegroundColor Cyan
...
try {
    ...
    Write-Host "Please run 'npm install -g pm2-windows-startup' and 'pm2-startup install' manually" -ForegroundColor Yellow
} finally {
    Pop-Location
}
